...
for v in data:
    ...
    if s1 == e1:
        ...
        # handle diagonals. I should have stopped and thought about how to handle
        # increasing/decreasing the x/y in lockstep but instead I ended up with this
        # horribly hacky thing that kept getting worse until I finally got it 
        ...
        if s2 > e2:
            factor = -1
        for i in range(s1, temp_end, temp):
            j = s2 + (count*factor)
            if j not in points[i]:
                points[i][j] = 0
            print(f"marking {i} {j}")
            points[i][j] += 1
            count += 1
