...
# Brute force look for one bingo then return the board number and sum of
# unmarked tiles
...
print(f"Num boards is {len(boards)}")
for c in calls:
    mark_number(boards, int(c))

    # note: multiple boards can bingo at once so be ready to delete multiple
    (board_num, bingo_sum) = find_bingo(boards)
    while bingo_sum != 0:
