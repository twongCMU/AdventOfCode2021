...
def parse_packet(cur_offset, b):
    ...
    if type_id == 4:
        ...
        while int(first_bit) == 1:
            ...
            full_value += value
        print(f"Final value is {int(full_value,2)}")
    else:
        ...
        if length_type_id == 0:
            ...
            while cur_offset < end_offset:
                (cur_offset, version_sum_temp) = parse_packet(cur_offset, b)
...
cur_offset = 0
v_num_sum = 0
while int(b[cur_offset:],2) > 0:
    (cur_offset, version_sum_temp) = parse_packet(cur_offset, b)
    v_num_sum += version_sum_temp

print(f"version sum {v_num_sum}")
