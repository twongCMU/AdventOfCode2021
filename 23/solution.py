...
def is_legal_up(graph, start_x, start_y, end_x):
    ...
    # if we are already home in row 2 or home in row 1 and row 2 is also correct, don't move
    if (start_x == index and start_y == 2) or (start_x == index and start_y == 1 and graph[2][start_x] == letter):
        return False

    # if we are in row 2 and somebody is above us
    if (start_y == 2 and graph[1][start_x] != "."):
        return False
...
def is_legal_down(graph, start_x, end_x, end_y):
    ...
    # if we move into our column at row 1, row 2 must be properly occupied already
    if end_y == 1 and (graph[1][end_x] != "." or graph[2][end_x] != letter):
        #print("row 2 not proper")
        return False

    # if we move into our column at row 2, row 2 must be empty
    if end_y == 2 and (graph[2][end_x] != "." or graph[1][end_x] != "."):
        #print("row 2 not open")
        return False
...
while len(states):
    ...
    for i in range(11):
        ...
        if is_legal_down(cur_graph, i, letter_index[letter], 2):
            (new_graph, new_score) = do_move(cur_graph, i, 0, letter_index[letter], 2)
            #print("appending")
            states.append((new_graph, new_score+cur_score))
